...
type GetStateHelpers<T> = T extends boolean
    ? Record<'toggle' | 'on' | 'off', () => void>
    : {}
        ...
        const listener = ({ code, metaKey }: KeyboardEvent) => {
            ...
            const moveFocus = (dir: 'prev' | 'next', absolute: boolean) => {
                const elemToFocus =
                    ...
                    (absolute ? getLastSiblingSelector : getSiblingSelector)(
                        activeElement!,
                        focusableElemSelector,
                        dir,
                    )
...
type UseAdvancedState = <T extends boolean | string | number>(
    initialValue: T | (() => T),
) => T & {
