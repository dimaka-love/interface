...
const filterValues = <K extends any>(
    obj: Record<string, K>,
...
> = (
    ...
    group: G,
    setting: SS,
) => TS[T][G][SS] extends { type: infer U } ? U : void
...
>(
    ...
    return (
        userSettingsState?.[tab]?.[group]?.[setting] ??
        tabsSchema[tab][group][setting]['defaultValue']
    )
}

/** This function **MUST** be called before AppProvider is used. Any suggestions on API improvements are welcome! */
export const initSettingsStore = <TS extends Record<string, SettingsSchema>>({
    localStorageKey,
    settingsTabsSchema,
}: InitSettingsStoreOptions<TS>) => {
    if (useSettingsStore) {
        ...
        persist(() => ({} as any), {
            ...
            serialize({ state, version }) {
                state = mapValues(state, (settingsGroups, tabName) => {
                    const settignsGroupsParsed = mapValues(
                        settingsGroups,
                        (settingsList, groupName) => {
                            return filterValues(settingsList, (key, value) => {
                                const settingSchema =
                                    settingsTabsSchema[tabName]![groupName]![
                                        key
                                    ]!
                                if (!('defaultValue' in settingSchema))
                                    return false
