...
let MenuWithTabs: React.FC<ComponentProps> = ({ tabs, tabsLabel }) => {
    ...
    useEventListener(window, "keydown", e => {
        if (!e.code.startsWith("Bracket") || tabsListRef.current.contains(document.activeElement)) return;
        const currentFocusedButton = tabsRootRef.current.querySelector(".Mui-selected") as HTMLElement;
        if (!currentFocusedButton) return;
        const elemToFocus = currentFocusedButton[e.code.endsWith("Left") ? "previousElementSibling" : "nextElementSibling"] as HTMLElement;
