...
export const PersistedSettingItem: React.FC<SettingItemCommonProps & Record<"tabName" | "groupName", string>> = ({ tabName, groupName, label, settingSchema }) => {
...
const SettingsItem: React.FC<SettingItemCommonProps & { onChange: (newValue: any) => unknown; value: any; }> = ({ label, settingSchema, onChange, value }) => {
    const handleFocusBlurEvents = useCallback(({ currentTarget, target, type: eventName }: React.FocusEvent<HTMLDivElement>) => {
    ...
    return <div
        ...
        onKeyDown={({ code, currentTarget, target, ...event }) => {
                ...
                case "Enter":
                    switch (settingSchema.type) {
                        case "menu":
                            ...
                            break;
                        case "slider":
                            currentTarget.querySelector('input[type="number"]')!.focus();
                            break;
                        case "toggle":
                            onChange(!value);
                            break;
            ...
            if (settingSchema.type === "slider") {
                ...
                onChange(
                    clamp(
                        value + (modifierKey ? 10 : 1) * (ArrowKey === "Right" ? 1 : -1),
                        settingSchema.min ?? 0,
